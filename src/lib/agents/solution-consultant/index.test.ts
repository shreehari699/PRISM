...
import { runSolutionConsultant } from "./index";
import { solutionConsultantOutputSchema } from "./schema";
...
const validTechnicalFeasibility = {
  modeFeasibility: {
    mode: "HACKATHON",
    hackathon: {
...
describe("runSolutionConsultant", () => {
  it("returns an error without calling the provider when Phase 02 output is missing", async () => {
    ...
    const result = await runSolutionConsultant(context({ stakeholder_pain: undefined }), provider);

    expect(result.status).toBe("error");
    expect(provider.generateStructured).not.toHaveBeenCalled();
  });

  it("returns an error without calling the provider when Phase 07 output is missing", async () => {
    const provider = fakeProvider({ status: "ok", model: "x", data: validOutput });

    const result = await runSolutionConsultant(
  ...
  it("calls the provider with the solution consultant schema when all upstream phases are valid", async () => {
    const provider = fakeProvider({ status: "ok", model: "x", data: validOutput });

    const result = await runSolutionConsultant(context(), provider);

    expect(result.status).toBe("ok");
    expect(provider.generateStructured).toHaveBeenCalledWith(
      expect.objectContaining({ schema: solutionConsultantOutputSchema }),
    );
  });
  ...
  it("passes the real Phase 06 market evidence source ids as sourceIdVocabulary", async () => {
    ...
    await runSolutionConsultant(context(), provider);
