...
import { runReportGenerator } from "./index";
...
describe("runReportGenerator", () => {
  it("calls the provider once all nine upstream phases validate", async () => {
    const provider = fakeProvider({ status: "unavailable", reason: "n/a" });

    await runReportGenerator(context(), provider);

    expect(provider.generateStructured).toHaveBeenCalledTimes(1);
  });

  it("returns an error without calling the provider when Phase 09 output is missing", async () => {
    const provider = fakeProvider({ status: "unavailable", reason: "n/a" });

    const result = await runReportGenerator(context({ poc_validation: undefined }), provider);
  ...
  it("returns an error without calling the provider when Phase 01 output is missing", async () => {
    ...
    const result = await runReportGenerator(
      context({ problem_intelligence: undefined }),
      provider,
    );
  ...
  it("propagates the provider's result status unchanged", async () => {
    const provider = fakeProvider({ status: "unavailable", reason: "model retired" });

    const result = await runReportGenerator(context(), provider);
