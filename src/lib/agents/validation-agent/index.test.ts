...
import { runValidationAgent } from "./index";
...
describe("runValidationAgent", () => {
  it("calls the provider once all eight upstream phases validate", async () => {
  ...
  it("propagates the provider's result status unchanged", async () => {
    ...
    const result = await runValidationAgent(context(), provider);
