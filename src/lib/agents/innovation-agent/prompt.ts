...
export function buildSystemInstruction(
  ...
  return [
    ...
    "",
    "CRITICAL — `differentiation.sourceIds` takes ONLY real research source ids from the 'Research sources' list below (e.g. source-3) — never an opportunity id, a gap id, or a pain id. If nothing in that list actually supports the differentiation claim, leave `sourceIds` empty and use INFERENCE or ASSUMPTION rather than VERIFIED with a fabricated citation.",
    "",
...
export function buildUserPrompt(
  ...
  const sourceLines =
    sources.length > 0
      ? sources
          ...
          .join("\n")
      : "(No research sources are available — `differentiation.sourceIds` must stay empty, and no differentiation claim may be VERIFIED.)";
  ...
  return [
    ...
    opportunityLines,
    "",
    "Research sources (Phase 03) — the ONLY valid values for `differentiation.sourceIds`:",
    sourceLines,
    "",
